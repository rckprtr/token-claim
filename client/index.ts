...
export class TokenClaim {
  ...
  createTokenClaimPDA(authority: PublicKey): PublicKey {
    const [tokenClaimsPDA] = PublicKey.findProgramAddressSync(
      [Buffer.from(TOKEN_CLAIMS_SEED), authority.toBuffer()],
      this.program.programId
    );
    return tokenClaimsPDA;
  }

  async getCreateInstruction(authority: PublicKey): Promise<Transaction> {
    return await this.program.methods
      .createTokenClaims()
      .accounts({
        authority: authority,
        tokenClaims: this.createTokenClaimPDA(authority),
      })
      .transaction();
  }
  ...
  async getDepositInstruction(
    ...
    const tokenClaimPDA = this.createTokenClaimPDA(authority);
  ...
  async getClaimInstruction(
    ...
    transaction.add(
     await this.program.methods
      .claimToken(new BN(nonce), new BN(amount))
      .accounts({
        authority: authority,
        receiver: receiver,
